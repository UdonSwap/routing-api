...
describe('QuoteHandler', () => {
  describe('.protocolsFromRequest', () => {
    it('returns V3 when no protocols are requested', () => {
      expect(QuoteHandler.protocolsFromRequest(ChainId.MODE, undefined, '', undefined, undefined)).toEqual([
        Protocol.V3,
      ])
    })

    it('returns V3 when forceCrossProtocols is false', () => {
      expect(QuoteHandler.protocolsFromRequest(ChainId.MODE, undefined, '', undefined, false)).toEqual([Protocol.V3])
    })

    it('returns empty when forceCrossProtocols is true', () => {
      expect(QuoteHandler.protocolsFromRequest(ChainId.MODE, undefined, '', undefined, true)).toEqual([])
    })

    it('returns requested protocols', () => {
      expect(
        QuoteHandler.protocolsFromRequest(ChainId.MODE, ['v2', 'v3', 'mixed'], '', undefined, undefined)
      ).toEqual([Protocol.V2, Protocol.V3, Protocol.MIXED])
    })

    it('returns a different set of requested protocols', () => {
      expect(QuoteHandler.protocolsFromRequest(ChainId.MODE, ['v3', 'mixed'], '', undefined, undefined)).toEqual([
        Protocol.V3,
        Protocol.MIXED,
      ])
    })
    ...
    it('works with other chains', () => {
      expect(QuoteHandler.protocolsFromRequest(ChainId.MODE, ['v2', 'v3', 'mixed'], '', undefined, undefined)).toEqual([
        Protocol.V2,
        Protocol.V3,
        Protocol.MIXED,
      ])
    ...
    it('returns undefined when a requested protocol is invalid', () => {
      expect(
        QuoteHandler.protocolsFromRequest(ChainId.MODE, ['v2', 'v3', 'mixed', 'miguel'], '', undefined, undefined)
      ).toBeUndefined()
    })
    ...
    describe('for mobile request', () => {
      ...
      //     expect(
      //       QuoteHandler.protocolsFromRequest(ChainId.MODE, ['v2', 'v3', 'mixed'], requestSource, undefined, undefined)
      //     ).toEqual([Protocol.V3])
      //   })
      // })
