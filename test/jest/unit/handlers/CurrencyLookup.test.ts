...
const token = new Token(ChainId.MODE, address, 18, 'FOO', 'Foo')
...
describe('CurrencyLookup', () => {
  it('Returns the native token if tokenRaw is a native token string', async () => {
    ...
    const result = await tokenLookup.searchForToken('ETH', ChainId.MODE)

    expect(result).toBeDefined()
    expect(result).toEqual(ExtendedEther.onChain(ChainId.MODE))
  })
  ...
  it('Finds the token in the token list when the input is an address', async () => {
    ...
    const result = await tokenLookup.searchForToken(address, ChainId.MODE)
  ...
  it('Finds the token in the token list when the input is a symbol', async () => {
    ...
    const result = await tokenLookup.searchForToken('FOO', ChainId.MODE)
