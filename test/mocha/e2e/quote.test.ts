...
import { ChainId, Currency, CurrencyAmount, ETHER, Fraction, Rounding, Token, WETH9 } from 'udonswap-core'
import {
  DAI_MODE,
  // CEUR_CELO,
  // CEUR_CELO_ALFAJORES,
...
import { DAI_ON, getAmount, getAmountFromToken, Abah072, USDC_ON, 
  // USDT_ON,
...
const UNIVERSAL_ROUTER_ADDRESS = UNIVERSAL_ROUTER_ADDRESS_BY_CHAIN(919)

if (!process.env.UDONSWAP_ROUTING_API || !process.env.ARCHIVE_NODE_RPC) {
  throw new Error('Must set UDONSWAP_ROUTING_API and ARCHIVE_NODE_RPC env variables for integ tests. See README')
}

const API = `${process.env.UDONSWAP_ROUTING_API!}quote`
...
const BULLET_WHT_TAX = new Token(
  ChainId.MODE,
  '0xF7ca2401709BC01Eba07d46c8d59e865C983e1AC', //LAMP
  8,
  'BULLET',
  'Bullet Game Betting Token',
  false,
...
const V2_SUPPORTED_PAIRS = [
  ...
  [WRAPPED_NATIVE_CURRENCY[ChainId.MODE], USDC_MODE],
]
...
export const agEUR_MAINNET = new Token(
  ChainId.MODE,
  '0x047D496B4DAFE62D28CE680862e62B0Fa55dBEf4', //YIN
  18,
  'agEUR',
  'agEur'
)

export const XSGD_MAINNET = new Token(ChainId.MODE, '0xC945456d07bced7FF5570e179Ee500b1F72aD0b4', 6, 'XSGD', 'XSGD') //MAH
...
describe('quote', function () {
  ...
  before(async function () {
    ...
    const quoteReq: QuoteQueryParams = {
      ...
      tokenOutChainId: 919,
      amount: await getAmount(919, 'exactIn', 'USDC', 'USDT', '100'),
      type: 'exactIn',
    }
    ...
    alice = await resetAndFundAtBlock(alice, block, [
      parseAmount('8000000', USDC_MODE),
      parseAmount('5000000', USDT_MODE),
      parseAmount('10', WBTC_MODE),
      parseAmount('1000', Abah072),
      parseAmount('4000', WETH9[919]),
      parseAmount('5000000', DAI_MODE),
    ...
    const aliceUSDCBalance = await getBalance(alice, USDC_MODE)
    expect(!aliceUSDCBalance.lessThan(parseAmount('8000000', USDC_MODE))).to.be.true
    const aliceUSDTBalance = await getBalance(alice, USDT_MODE)
    expect(!aliceUSDTBalance.lessThan(parseAmount('5000000', USDT_MODE))).to.be.true
    const aliceWETH9Balance = await getBalance(alice, WETH9[919])
    expect(!aliceWETH9Balance.lessThan(parseAmount('4000', WETH9[919]))).to.be.true
    const aliceWBTCBalance = await getBalance(alice, WBTC_MODE)
    expect(!aliceWBTCBalance.lessThan(parseAmount('10', WBTC_MODE))).to.be.true
    const aliceDAIBalance = await getBalance(alice, DAI_MODE)
    expect(!aliceDAIBalance.lessThan(parseAmount('5000000', DAI_MODE))).to.be.true
    const aliceUNIBalance = await getBalance(alice, Abah072)
    expect(!aliceUNIBalance.lessThan(parseAmount('1000', Abah072))).to.be.true
      ...
      describe(`${ID_TO_NETWORK_NAME(1)} ${algorithm} ${type} 2xx`, () => {
        describe(`+ Execute Swap`, () => {
          it(`erc20 -> erc20`, async () => {
            const quoteReq: QuoteQueryParams = {
              ...
              tokenOutAddress: 'USDT',
              tokenOutChainId: 919,
              amount: await getAmount(919, type, 'USDC', 'USDT', '100'),
              type,
            ...
            if (type == 'exactIn') {
              ...
              checkQuoteToken(tokenOutBefore, tokenOutAfter, CurrencyAmount.fromRawAmount(USDT_MODE, quote))
            } else {
          ...
          it(`erc20 -> erc20 swaprouter02`, async () => {
            ...
            expect(methodParameters).to.not.be.undefined
            expect(methodParameters?.to).to.equal(SWAP_ROUTER_02_ADDRESSES(ChainId.MODE))
          ...
          it(`erc20 -> erc20 with permit`, async () => {
            const amount = await getAmount(919, type, 'USDC', 'USDT', '10')

            const nonce = nextPermitNonce()

            const permit: PermitSingle = {
              details: {
                token: USDC_MODE.address,
          ...
          it(`erc20 -> eth`, async () => {
            const quoteReq: QuoteQueryParams = {
              ...
              tokenOutAddress: 'ETH',
              tokenOutChainId: 919,
              amount: await getAmount(919, type, 'USDC', 'ETH', type == 'exactIn' ? '1000000' : '10'),
              type,
              recipient: alice.address,
              slippageTolerance: SLIPPAGE,
              deadline: '360',
          ...
          it(`erc20 -> eth large trade`, async () => {
            ...
            const quoteReq: QuoteQueryParams = {
              ...
              amount:
                type == 'exactIn'
                  ? await getAmount(919, type, 'USDC', 'ETH', '1000000')
                  : await getAmount(919, type, 'USDC', 'ETH', '100'),
              type,
              recipient: alice.address,
              slippageTolerance: SLIPPAGE,
              deadline: '360',
          ...
          it(`eth -> erc20`, async () => {
            const quoteReq: QuoteQueryParams = {
              ...
              amount:
                type == 'exactIn'
                  ? await getAmount(919, type, 'ETH', 'UNI', '10')
                  : await getAmount(919, type, 'ETH', 'UNI', '10000'),
              type,
              recipient: alice.address,
              slippageTolerance: SLIPPAGE,
              deadline: '360',
          ...
          it(`eth -> erc20 swaprouter02`, async () => {
            ...
            expect(status).to.equal(200)
            expect(data.methodParameters).to.not.be.undefined
            expect(data.methodParameters?.to).to.equal(SWAP_ROUTER_02_ADDRESSES(ChainId.MODE))
          ...
          it(`weth -> erc20`, async () => {
            const quoteReq: QuoteQueryParams = {
              ...
              tokenOutAddress: 'DAI',
              tokenOutChainId: 919,
              amount: await getAmount(919, type, 'WETH', 'DAI', '100'),
              type,
              recipient: alice.address,
              slippageTolerance: SLIPPAGE,
              deadline: '360',
            ...
            if (type == 'exactIn') {
              expect(tokenInBefore.subtract(tokenInAfter).toExact()).to.equal('100')
              checkQuoteToken(tokenOutBefore, tokenOutAfter, CurrencyAmount.fromRawAmount(DAI_MODE, data.quote))
            } else {
              expect(tokenOutAfter.subtract(tokenOutBefore).toExact()).to.equal('100')
              checkQuoteToken(tokenInBefore, tokenInAfter, CurrencyAmount.fromRawAmount(WETH9[919]!, data.quote))
            }
          })
          ...
          it(`erc20 -> weth`, async () => {
            const quoteReq: QuoteQueryParams = {
              ...
              tokenOutChainId: 919,
              amount: await getAmount(919, type, 'USDC', 'WETH', '100'),
              type,
              recipient: alice.address,
              slippageTolerance: SLIPPAGE,
              deadline: '360',
            ...
            if (type == 'exactIn') {
              expect(tokenInBefore.subtract(tokenInAfter).toExact()).to.equal('100')
              checkQuoteToken(tokenOutBefore, tokenOutAfter, CurrencyAmount.fromRawAmount(WETH9[919], data.quote))
            } else {
              expect(tokenOutAfter.subtract(tokenOutBefore).toExact()).to.equal('100')
              checkQuoteToken(tokenInBefore, tokenInAfter, CurrencyAmount.fromRawAmount(USDC_MODE, data.quote))
            }
          ...
          if (algorithm == 'alpha') {
            ...
            if (type === 'exactIn') {
              it.skip(`erc20 -> erc20 forceMixedRoutes not specified for v2,v3 does not return mixed route even when it is better`, async () => {
                const quoteReq: QuoteQueryParams = {
                  ...
                  tokenOutAddress: 'APE',
                  tokenOutChainId: 919,
                  amount: await getAmount(919, type, 'BOND', 'APE', '10000'),
                  type,
                  recipient: alice.address,
                  slippageTolerance: SLIPPAGE,
                  deadline: '360',
            ...
            if (type === 'exactIn') {
              const tokenInAndTokenOut = [
                [BULLET, WETH9[ChainId.MODE]!],
                [WETH9[ChainId.MODE]!, BULLET],
              ]
              ...
              tokenInAndTokenOut.forEach(([tokenIn, tokenOut]) => {
                ...
                it(`fee-on-transfer ${tokenIn.symbol} -> ${tokenOut.symbol}`, async () => {
                  ...
                  const originalAmount = tokenIn.equals(WETH9[ChainId.MODE]!) ? '10' : '2924'
                  ...
                  const responses = await Promise.all(
                    enableFeeOnTransferFeeFetching.map(async (enableFeeOnTransferFeeFetching) => {
                      ...
                      const simulateFromAddress = tokenIn.equals(WETH9[ChainId.MODE]!)
        ...
        it(`erc20 -> erc20 by address`, async () => {
          const quoteReq: QuoteQueryParams = {
            ...
            tokenInChainId: 919, // DAI
            tokenOutAddress: '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48',
            tokenOutChainId: 919, // USDC
            amount: await getAmount(919, type, 'DAI', 'USDC', '100'),
            type,
            recipient: alice.address,
            slippageTolerance: SLIPPAGE,
            deadline: '360',
      ...
      describe(`${ID_TO_NETWORK_NAME(1)} ${algorithm} ${type} 4xx`, () => {
        ...
        it.skip(`amount is too big to find route`, async () => {
          const quoteReq: QuoteQueryParams = {
            ...
            tokenOutAddress: 'KNC',
            tokenOutChainId: 919,
            amount: await getAmount(919, type, 'UNI', 'KNC', '9999999999999999999999999999999999999999999999999'),
            type,
        ...
        it(`tokens are the same symbol and address`, async () => {
          const quoteReq: QuoteQueryParams = {
            ...
            tokenOutChainId: 919,
            amount: await getAmount(919, type, 'USDT', 'USDT', '100'),
            type,
            recipient: alice.address,
            slippageTolerance: SLIPPAGE,
            deadline: '360',
        ...
        it(`tokens are the same after getting wrapped`, async () => {
          const quoteReq: QuoteQueryParams = {
            ...
            tokenOutChainId: 919,
            amount: await getAmount(919, type, 'ETH', 'WETH', '1'),
            type,
            recipient: alice.address,
            slippageTolerance: SLIPPAGE,
            deadline: '360',
        ...
        it(`recipient is an invalid address`, async () => {
          const quoteReq: QuoteQueryParams = {
            ...
            tokenOutAddress: 'USDC',
            tokenOutChainId: 919,
            amount: await getAmount(919, type, 'USDT', 'USDC', '100'),
            type,
    ...
    for (const type of TRADE_TYPES) {
      ...
      describe(`${ID_TO_NETWORK_NAME(chain)} ${type} 2xx`, function () {
        ...
        it(`${wrappedNative.symbol} -> erc20`, async () => {
          ...
          const amount = type === 'exactOut' && chain === ChainId.MODE ? '0.002' : '1'
        ...
        it(`${native} -> erc20`, async () => {
          ...
          const baseErc20 = chain == ChainId.MODE ? USDC_ON(ChainId.MODE) : erc2
