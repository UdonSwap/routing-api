...
describe('ProdConfig', () => {
  it('test generate json string from ProdConfig', () => {
    ...
    expect(jsonStr).equal(
      '[{"chainId":919,"useMultiProviderProb":0}]'
      // ,{"chainId":43114,"useMultiProviderProb":1,"sessionAllowProviderFallbackWhenUnhealthy":true,"providerInitialWeights":[-1,-1],"providerUrls":["url1","url2"]}
    )
  })
  ...
  it('test parse json string into ProdConfig with validation, good case', () => {
    const jsonStr =
      '[{"chainId":919,"useMultiProviderProb":0}]'
      // ,{"chainId":43114,"useMultiProviderProb":1,"sessionAllowProviderFallbackWhenUnhealthy":true,"providerInitialWeights":[-1,-1],"providerUrls":["url1","url2"],"enableDbSync": true}]'
    ...
    expect(prodConfig.length).equal(1)
  ...
  it('test getRpcGatewayEnabledChainIdProviderNamePairs', () => {
    ...
    expect(enabledChains.get(ChainId.MODE)).to.have.members(['RPC'])
  })
