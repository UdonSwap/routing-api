...
const createNewSingleJsonRpcProviders = () => [
  new SingleJsonRpcProvider(
    { name: 'mode', chainId: ChainId.MODE },
    `url_0`,
    log,
...
const SINGLE_RPC_PROVIDERS = { [ChainId.MODE]: createNewSingleJsonRpcProviders() }

const resetRpcProviders = () => {
  SINGLE_RPC_PROVIDERS[ChainId.MODE] = createNewSingleJsonRpcProviders()
}
...
describe('UniJsonRpcProvider', () => {
  ...
  beforeEach(() => {
    uniProvider = new UniJsonRpcProvider(
      ChainId.MODE,
      SINGLE_RPC_PROVIDERS[ChainId.MODE],
      log,
