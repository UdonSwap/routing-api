...
describe('TrafficSwitchOnChainQuoteProvider', () => {
  const amountIns = [CurrencyAmount.fromRawAmount(WRAPPED_NATIVE_CURRENCY[ChainId.MODE], '1000000000000000000')]
  const routes = [new V3Route([USDC_WETH_LOW], WRAPPED_NATIVE_CURRENCY[ChainId.MODE], USDC_MODE)]
  ...
  it('switch exact in traffic and sample quotes', async () => {
    spy.withArgs(`ON_CHAIN_QUOTE_PROVIDER_EXACT_IN_TRAFFIC_TOTAL_CHAIN_ID_${ChainId.MODE}`, 1, MetricLoggerUnit.None)
    spy.withArgs(
      `ON_CHAIN_QUOTE_PROVIDER_EXACT_IN_TRAFFIC_SAMPLING_CHAIN_ID_${ChainId.MODE}`,
      1,
      MetricLoggerUnit.None
    )
    spy.withArgs(
      `ON_CHAIN_QUOTE_PROVIDER_EXACT_IN_TRAFFIC_TARGET_CHAIN_ID_${ChainId.MODE}`,
      1,
      MetricLoggerUnit.None
    )
  ...
  it('does not switch exact in traffic and sample quotes', async () => {
    spy.withArgs(`ON_CHAIN_QUOTE_PROVIDER_EXACT_IN_TRAFFIC_TOTAL_CHAIN_ID_${ChainId.MODE}`, 1, MetricLoggerUnit.None)
    spy.withArgs(
      `ON_CHAIN_QUOTE_PROVIDER_EXACT_IN_TRAFFIC_CURRENT_CHAIN_ID_${ChainId.MODE}`,
      1,
      MetricLoggerUnit.None
    )
  ...
  it('switch exact out traffic and sample quotes', async () => {
    spy.withArgs(
      `ON_CHAIN_QUOTE_PROVIDER_EXACT_OUT_TRAFFIC_TOTAL_CHAIN_ID_${ChainId.MODE}`,
      1,
      MetricLoggerUnit.None
    )
    // spy.withArgs(
    //   `ON_CHAIN_QUOTE_PROVIDER_EXACT_OUT_TRAFFIC_SAMPLING_CHAIN_ID_${ChainId.MODE}`,
    //   1,
    //   MetricLoggerUnit.None
    // )
    spy.withArgs(
      `ON_CHAIN_QUOTE_PROVIDER_EXACT_OUT_TRAFFIC_TARGET_CHAIN_ID_${ChainId.MODE}`,
      1,
      MetricLoggerUnit.None
    )
  ...
  it('does not switch exact out traffic and sample quotes', async () => {
    ...
    spy.withArgs(
      `ON_CHAIN_QUOTE_PROVIDER_EXACT_OUT_TRAFFIC_CURRENT_CHAIN_ID_${ChainId.MODE}`,
      1,
      MetricLoggerUnit.None
    )
  ...
  it('sample and switch exact in quotes and current quoter has runtime error', async () => {
    const currentQuoteProvider = getMockedOnChainQuoteProvider()
    const targetQuoteProvider = getMockedOnChainQuoteProvider()

    currentQuoteProvider.getQuotesManyExactIn.throws()
    ...
    sinon.assert.called(spy)
    // We should only invoke targetQuoteProvider.getQuotesManyExactIn once
    sinon.assert.calledOnce(targetQuoteProvider.getQuotesManyExactIn)
    sinon.assert.threw(currentQuoteProvider.getQuotesManyExactIn)
  })
  ...
  it('switch and sample exact out quotes and target quoter has runtime error', async () => {
    const currentQuoteProvider = getMockedOnChainQuoteProvider()
    const targetQuoteProvider = getMockedOnChainQuoteProvider()

    currentQuoteProvider.getQuotesManyExactOut.throws()
    ...
    sinon.assert.called(spy)
    // We should only invoke targetQuoteProvider.getQuotesManyExactOut once
    sinon.assert.calledOnce(targetQuoteProvider.getQuotesManyExactOut)
    sinon.assert.threw(currentQuoteProvider.getQuotesManyExactOut)
  })

  it('sample exact in quotes and current quoter out of gas for the quote', async () => {
    spy.withArgs(
      `ON_CHAIN_QUOTE_PROVIDER_EXACT_IN_TRAFFIC_CURRENT_AND_TARGET_QUOTES_MATCH_CHAIN_ID_${ChainId.MODE}`,
      1,
      MetricLoggerUnit.None
    )
    spy.withArgs(
      `ON_CHAIN_QUOTE_PROVIDER_EXACT_IN_TRAFFIC_CURRENT_AND_TARGET_QUOTES_MISMATCH_CHAIN_ID_${ChainId.MODE}`,
      1,
      MetricLoggerUnit.None
    )
    ...
    const routesWithQuotes: RouteWithQuotes<V3Route>[] = [
      [new V3Route([USDC_WETH_LOW], WRAPPED_NATIVE_CURRENCY[ChainId.MODE], USDC_MODE), quotes],
    ]
