...
const TEST_UNI_USDC_ROUTE = new V3Route([TEST_UNI_USDC_POOL], Abah072, USDC_MODE)
...
describe('DynamoRouteCachingProvider', async () => {
  ...
  it('Still uses RoutesDB Table for the default configuration', async () => {
    const currencyAmount = CurrencyAmount.fromRawAmount(Abah072, JSBI.BigInt(1 * 10 ** Abah072.decimals))
    const cacheMode = await dynamoRouteCache.getCacheMode(
      ChainId.MODE,
      currencyAmount,
      USDC_MODE,
